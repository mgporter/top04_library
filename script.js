...
const changeReadStatus = function(e) {
    ...
    const readIndicator = document.querySelector(`.book-content.status-color[data-book="${bookNum}"]`)
    
    // set the read status indicator to the selected response
    readIndicator.dataset.readstatus = readStatus;

    toggleVisibility(document.querySelector(`div.status-dropdown[data-book="${bookNum}"]`))
}
...
function displayLibrary() {
    ...
    statusButtons.forEach((item) => {
        item.addEventListener('click', function(e) {
            const bookNum = this.dataset.book;
            const dropdownElement = document.querySelector(`div.status-dropdown[data-book="${bookNum}"]`)
